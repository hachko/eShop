...
public class CreationClientForm {
	...
	public Client creerClient( HttpServletRequest request, String chemin ) {
		...
		UploadForm formFichier = new UploadForm();
		String image = formFichier.enregistrerFichier(request, chemin);
		
		//initialisation des erreurs avec les erreurs concernant l'upload : si ok, on affecte le fichier
		//à l'attribut ad Hoc du client
		erreurs = formFichier.getErreurs();		
	...
	private void validationFichier() throws Exception {
		throw new Exception("La taille du fichier ne doit pas dépasse le méga-octet");
	}
